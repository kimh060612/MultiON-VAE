...
@baseline_registry.register_trainer(name="trajectory-attention")
class PPOTrainerTrajUncertain(BaseRLTrainerTrajUncertain):
    r"""Trainer class for PPO algorithm
    Paper: https://arxiv.org/abs/1707.06347.
    """
    supported_tasks = ["Nav-v0"]
    ...
    def _setup_actor_critic_agent(self, ppo_cfg: Config) -> None:
        ...
        self.actor_critic = BaselinePolicyTrajUncertain(
            batch_size=self.config.NUM_PROCESSES,
            observation_space=self.envs.observation_spaces[0],
            action_space=self.envs.action_spaces[0],
            hidden_size=ppo_cfg.hidden_size,
            ...
            egocentric_map_size=self.config.RL.MAPS.egocentric_map_size,
            global_map_size=self.config.RL.MAPS.global_map_size,
            global_crop_size=self.config.RL.MAPS.global_crop_size,
            global_map_depth=self.config.RL.MAPS.global_map_depth,
            coordinate_min=self.config.RL.MAPS.coordinate_min,
            coordinate_max=self.config.RL.MAPS.coordinate_max
        )
        ...
        self.agent = PPOTrajUncertain(
            actor_critic=self.actor_critic,
            clip_param=ppo_cfg.clip_param,
            ppo_epoch=ppo_cfg.ppo_epoch,
            num_mini_batch=ppo_cfg.num_mini_batch,
            num_mini_steps=ppo_cfg.num_mini_steps,
            value_loss_coef=ppo_cfg.value_loss_coef,
            entropy_coef=ppo_cfg.entropy_coef,
            lr=ppo_cfg.lr,
            eps=ppo_cfg.eps,
    ...
    def _collect_rollout_step(
        ...
        with torch.no_grad():
            ...
            (
                ...
                rollouts.recurrent_hidden_states[rollouts.step],
                rollouts.global_map[rollouts.step],
                rollouts.trajectory_map[rollouts.step],
                rollouts.prev_actions[rollouts.step],
                rollouts.masks[rollouts.step],
            )
    ...
    def train(self) -> None:
        ...
        rollouts = RolloutStorageTrajectory(
            ppo_cfg.num_steps,
            self.envs.num_envs,
            self.envs.observation_spaces[0],
            self.envs.action_spaces[0],
            ppo_cfg.hidden_size,
            self.config.RL.MAPS.global_map_size,
            self.config.RL.MAPS.global_crop_size,
            self.config.RL.MAPS.global_map_depth,
        )
        rollouts.to(self.device)
