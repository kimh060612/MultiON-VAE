...
from habitat_baselines.common.base_trainer import BaseRLTrainerNonOracle, BaseRLTrainerOracle, BaseRLTrainerExploration
from habitat_baselines.common.baseline_registry import baseline_registry
from habitat_baselines.common.env_utils import construct_envs
from habitat_baselines.common.environments import get_env_class
from habitat_baselines.common.rollout_storage import RolloutStorageOracle, RolloutStorageNonOracle, RolloutStorageExploration
...
from habitat_baselines.rl.ppo import PPONonOracle, PPOOracle, BaselinePolicyNonOracle, BaselinePolicyOracle, BaselinePolicyExploration, PPOExploration
        ...
        while (
            ...
            if self.config.EVAL.GATHER_MAP_DATA and not (self.config.EVAL.MAP_DATA_PATH == "") and (episode_steps % self.config.RL.MAPS.map_collection_stride == 0):
                ...
                for eidx in range(bs):
                    np.save(f"{self.config.EVAL.MAP_DATA_PATH}allo_#id{self.map_id}.npy", np_allo_map[eidx, ...])
                    np.save(f"{self.config.EVAL.MAP_DATA_PATH}ego_#id{self.map_id}.npy", np_ego_map[eidx, ...])
                    # Save Depth map for debug
                    np.save(f"{self.config.EVAL.MAP_DATA_PATH}depth_#id{self.map_id}.npy", np_depth[eidx, ...])
