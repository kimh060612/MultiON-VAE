...
from habitat_baselines.common.base_trainer import BaseRLTrainerNonOracle, BaseRLTrainerOracle, BaseRLTrainerExploration, BaseRLTrainerExpAttention
from habitat_baselines.common.baseline_registry import baseline_registry
from habitat_baselines.common.env_utils import construct_envs
from habitat_baselines.common.environments import get_env_class
from habitat_baselines.common.rollout_storage import RolloutStorageOracle, RolloutStorageNonOracle, RolloutStorageExploration, RolloutStorageExpAttention
...
from habitat_baselines.rl.ppo import PPONonOracle, PPOOracle, BaselinePolicyNonOracle, BaselinePolicyOracle, BaselinePolicyExploration, PPOExploration, PPOExpAttention, BaselinePolicyExpAttention
...
class PPOTrainerNO(BaseRLTrainerNonOracle):
    ...
    def _eval_checkpoint(
        ...
        map_generator = OccupancyMap(
            self.config.RL.MAPS.global_map_size,
            self.config.RL.MAPS.global_egocentric_map_size,
            self.config.NUM_PROCESSES,
            self.device,
            self.config.RL.MAPS.coordinate_min,
            self.config.RL.MAPS.coordinate_max,
            self.config.RL.MAPS.vaccant_belief,
            self.config.RL.MAPS.occupied_belief,
        ) if self.config.EVAL.GATHER_MAP_DATA and not (self.config.EVAL.MAP_DATA_PATH == "") else None
        ...
        while (
            ...
            ## test_global_map: need to check whether it is.
            ### index i: i-th environment's index
            ### timestep: episode's current timestep.
            ### RGB image: observations[i]['rgb']
            ### heading: observations[i]['compass']
            ### position: observations[i]['gps']
            
            ### TODO: Save tuple (Egocentric map, Pose, Allocentric Map) in a database.
            ###     : Only save map in evaluation & save mode
            if self.config.EVAL.GATHER_MAP_DATA and not (self.config.EVAL.MAP_DATA_PATH == ""): # and (episode_steps % self.config.RL.MAPS.map_collection_stride == 0):
                # TODO: Collecting Global Map Data to compare the egocentric map!
                # Check List
                ## 1. Whether there exists noise in the sensor
                ## 2. Whether we can directly collect the global map data
                ## 3. Whether the Occupancy map collected correctly.  
                
                observation_batch = batch_obs(observations, device=self.device)
                # print(observation_batch.keys())
                map_generator.update_map(observation_batch)
                (
                    global_allocentric_map,
                    global_egocentric_map
                ) = map_generator.get_current_global_maps()
                np_allo_map: np.ndarray = global_allocentric_map.cpu().numpy()
                np_ego_map: np.ndarray = global_egocentric_map.cpu().numpy()
                np_depth: np.ndarray = observation_batch['depth'].cpu().numpy()
                np_rgb: np.ndarray = observation_batch['rgb'].cpu().numpy()
                bs = np_allo_map.shape[0]
                pose_list = {
                    "id": [],
                    "pose": [],
                    "episode_id": []
                ...
                for eidx in range(bs):
                    epi_id = current_episodes[eidx].episode_id
                    np.save(f"{self.config.EVAL.MAP_DATA_PATH}allo_#id{self.map_id}_#eid{epi_id}.npy", np_allo_map[eidx, ...])
                    np.save(f"{self.config.EVAL.MAP_DATA_PATH}ego_#id{self.map_id}_#eid{epi_id}.npy", np_ego_map[eidx, ...])
                    # Save Depth map for debug
                    np.save(f"{self.config.EVAL.MAP_DATA_PATH}depth_#id{self.map_id}_#eid{epi_id}.npy", np_depth[eidx, ...])
                    np.save(f"{self.config.EVAL.MAP_DATA_PATH}rgb_#id{self.map_id}_#eid{epi_id}.npy", np_rgb[eidx, ...])
                    pose_list["id"].append(self.map_id)
                    pose_list["pose"].append(np.array([
                        observation_batch['gps'][eidx, 0].item(),
                        observation_batch['gps'][eidx, 1].item(),
                        observation_batch['compass'][eidx].item()
                    ]))
                    pose_list["episode_id"].append(epi_id)
                    self.map_id += 1
                    
                    observation_batch['gps'] = observation_batch['gps'].to(torch.device('cpu'))
                    observation_batch['rgb'] = observation_batch['rgb'].to(torch.device('cpu'))
                    observation_batch['depth'] = observation_batch['depth'].to(torch.device('cpu'))
                    observation_batch['compass'] = observation_batch['compass'].to(torch.device('cpu'))
                    grid_x, grid_y = to_grid(
                        map_config.coordinate_min,
                        map_config.coordinate_max,
                        map_config.global_map_size,
                        observation_batch['gps'][eidx]
                    )
                    projection1 = draw_projection(observation_batch['rgb'][eidx], observation_batch['depth'][eidx] * 10, map_config.egocentric_map_size, map_config.global_map_size, map_config.coordinate_min, map_config.coordinate_max)
                    projection1 = projection1.squeeze(0).squeeze(0).permute(1, 2, 0)

                    projection1 = rotate_tensor(projection1.permute(2, 0, 1).unsqueeze(0), torch.tensor(-(observation_batch["compass"][eidx])).unsqueeze(0))
                    projection1 = projection1.squeeze(0).permute(1, 2, 0)

                    s = map_config.egocentric_map_size
                    temp = torch.max(test_global_map_visualization[eidx][grid_x - math.floor(s/2):grid_x + math.ceil(s/2),grid_y - math.floor(s/2):grid_y + math.ceil(s/2),:], projection1)
                    test_global_map_visualization[eidx][grid_x - math.floor(s/2):grid_x + math.ceil(s/2),grid_y - math.floor(s/2):grid_y + math.ceil(s/2),:] = temp
                    global_map1 = rotate_tensor(test_global_map_visualization[eidx][grid_x - math.floor(51/2):grid_x + math.ceil(51/2),grid_y - math.floor(51/2):grid_y + math.ceil(51/2),:].permute(2, 1, 0).unsqueeze(0), torch.tensor(-(observation_batch["compass"][eidx])).unsqueeze(0)).squeeze(0).permute(1, 2, 0).numpy()
                    np.save(f"{self.config.EVAL.MAP_DATA_PATH}global_#id{self.map_id}_#eid{epi_id}.npy", global_map1)
                    
                df = pd.DataFrame(pose_list)
                df.to_csv(f"{self.config.EVAL.MAP_DATA_PATH}/map_pose_info.csv", index=False, mode='a', header=False)
...
@baseline_registry.register_trainer(name="exploration-multion")
class PPOTrainerExploration(BaseRLTrainerExploration):
    r"""Trainer class for PPO algorithm
    Paper: https://arxiv.org/abs/1707.06347.
    """
    supported_tasks = ["Nav-v0"]
    ...
    def _setup_actor_critic_agent(self, ppo_cfg: Config) -> None:
        ...
        self.entropy_lambda = ppo_cfg.entropy_weight
        self.vae_encoer = Encoder(1, ch=64, blocks=(1, 2, 4, 8), latent_channels=16,
                                num_res_blocks=1, norm_type="bn", deep_model=True)
        checkpoint = torch.load(self.config.RL.MAPS.map_checkpoint_path + "Models/MAP_VAE_256_encoder.pt")
        self.vae_encoer.load_state_dict(checkpoint['model_state_dict'])
        self.vae_encoer = self.vae_encoer.eval().to(self.device)
        self.actor_critic = BaselinePolicyExploration(
            batch_size=self.config.NUM_PROCESSES,
            observation_space=self.envs.observation_spaces[0],
            action_space=self.envs.action_spaces[0],
            hidden_size=ppo_cfg.hidden_size,
            ...
            coordinate_min=self.config.RL.MAPS.coordinate_min,
            coordinate_max=self.config.RL.MAPS.coordinate_max,
            encoder=self.vae_encoer
        )
        self.actor_critic.to(self.device)

        self.agent = PPOExploration(
            actor_critic=self.actor_critic,
            clip_param=ppo_cfg.clip_param,
            ppo_epoch=ppo_cfg.ppo_epoch,
            num_mini_batch=ppo_cfg.num_mini_batch,
    ...
    def _collect_rollout_step(
        ...
        with torch.no_grad():
            ...
            (
                ...
                step_observation,
                rollouts.recurrent_hidden_states[rollouts.step],
                rollouts.occupancy_map[rollouts.step],
                rollouts.prev_actions[rollouts.step],
                rollouts.masks[rollouts.step],
            )
        ...
        prev_map_entropy = rollouts.map_reward_entropy[rollouts.step]
        # print(rewards.shape, map_reward_entropy.shape)
        rewards = (
            rewards + self.entropy_lambda * (
                map_reward_entropy - prev_map_entropy
            ).squeeze(1).to(current_episode_reward.device)
        ).unsqueeze(1)
    ...
    def train(self) -> None:
        ...
        rollouts = RolloutStorageExploration(
            ppo_cfg.num_steps,
            self.envs.num_envs,
            self.envs.observation_spaces[0],
            self.envs.action_spaces[0],
            ppo_cfg.hidden_size,
            self.config.RL.MAPS.global_map_size,
            egocentric_map_size=self.config.RL.MAPS.global_egocentric_map_size,
            coordinate_min=self.config.RL.MAPS.coordinate_min,
            coordinate_max=self.config.RL.MAPS.coordinate_max,
            vaccant_belief=self.config.RL.MAPS.vaccant_belief,
            occupied_belief=self.config.RL.MAPS.occupied_belief
        )
        rollouts.to(self.device)
...
@baseline_registry.register_trainer(name="exploration-attention")
class PPOTrainerExpAttn(BaseRLTrainerExpAttention):
    r"""Trainer class for PPO algorithm
    Paper: https://arxiv.org/abs/1707.06347.
    """
    supported_tasks = ["Nav-v0"]
    ...
    def _setup_actor_critic_agent(self, ppo_cfg: Config) -> None:
        ...
        self.actor_critic = BaselinePolicyExpAttention(
            batch_size=self.config.NUM_PROCESSES,
            observation_space=self.envs.observation_spaces[0],
            action_space=self.envs.action_spaces[0],
            hidden_size=ppo_cfg.hidden_size,
            ...
            coordinate_min=self.config.RL.MAPS.coordinate_min,
            coordinate_max=self.config.RL.MAPS.coordinate_max,
            camera_height=self.config.TASK_CONFIG.SIMULATOR.DEPTH_SENSOR.POSITION[1]
        )
        self.actor_critic.to(self.device)
        self.map_reward_coeff = ppo_cfg.map_reward_coeff
        self.agent = PPOExpAttention(
            actor_critic=self.actor_critic,
            clip_param=ppo_cfg.clip_param,
            ppo_epoch=ppo_cfg.ppo_epoch,
            num_mini_batch=ppo_cfg.num_mini_batch,
    ...
    def _collect_rollout_step(
        ...
        t_update_stats = time.time()
        batch = batch_obs(observations, device=self.device)
        prev_map_reward = rollouts.map_reward[rollouts.step]
        rewards = torch.tensor(
            rewards, dtype=torch.float, device=current_episode_reward.device
        )
        ...
        rewards = (
            rewards + 
            self.map_reward_coeff * (map_reward - prev_map_reward).squeeze(1).to(current_episode_reward.device)
        ).unsqueeze(1)
    ...
    def train(self) -> None:
        ...
        rollouts = RolloutStorageExpAttention(
            ppo_cfg.num_steps,
            self.envs.num_envs,
            self.envs.observation_spaces[0],
            self.envs.action_spaces[0],
    ...
    def _eval_checkpoint(
        ...
        while (
            ...
            with torch.no_grad():
                ...
                prev_actions.copy_(actions)
                diff_reward = test_map_reward - prev_map_reward
                prev_map_reward.copy_(test_map_reward)
            ...
            rewards = rewards + diff_reward * self.map_reward_coeff
            current_episode_reward += rewards
            next_episodes = self.envs.current_episodes()
            envs_to_pause = []
            n_envs = self.envs.num_envs
