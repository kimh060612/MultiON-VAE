...
def main():
    ...
    parser.add_argument(
        "--agent-type",
        choices=["no-map", "oracle", "oracle-ego", "proj-neural", "obj-recog", "exp-multinav", "ea-multinav", "traj-multinav"],
        required=True,
        help="agent type: oracle, oracleego, projneural, objrecog, expmultinav, eamultinav, trajmultinav",
    )
...
def run_exp(exp_config: str, run_type: str, agent_type: str, opts=None) -> None:
    ...
    if agent_type in ["oracle", "oracle-ego", "no-map"]:
        ...
        config.freeze()
    elif agent_type in ["proj-neural", "obj-recog"]:
        ...
        config.RL.PPO.hidden_size = 512
        config.freeze()
    elif agent_type == "traj-multinav":
        trainer_init = baseline_registry.get_trainer("trajectory-attention")
        config.defrost()
        config.RL.PPO.hidden_size = 512
        config.freeze()
